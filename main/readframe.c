...
static void sd_reader_task(void *arg)
{
    while (running) {
        ...
        if (cmd == CMD_RESET) {
            ...
            cmd = CMD_NONE;
            seek_idx = 0;  // 重置 seek_idx
        }

        /* ---- read one frame ---- */
        esp_err_t err = frame_reader_read(&frame_buf);  // 使用 frame_buf
...
esp_err_t read_frame(table_frame_t *playerbuffer)
{
    ...
    // 從 frame_buf 複製數據到 playerbuffer
...
uint8_t frame_get_led_num(uint8_t strip_index) {
    if (strip_index >= g_strip_num || g_led_num == NULL) return 0;
    return g_led_num[strip_index];
}

uint32_t frame_get_frame_num(void) {
    return g_ctrl.frame_num;
}

uint32_t frame_get_timestamp(uint32_t frame_index) {
    if (frame_index >= g_ctrl.frame_num || !g_ctrl.timestamps) return 0;
    return g_ctrl.timestamps[frame_index];
}