...
void frame_test_task(void *pvParameters){
    ...
    ESP_LOGI(TAG, "Configuration: OF=%d, Strips=%d, Total frames=%lu", of_num, strip_num, frame_num);
    ...
    while (1){
        ...
        if (frame_count == 1) || (frame_count % 100 == 0){
            ...
            for (int i = 0; i < of_num; i++) {
                ...
                if (i % 10 == 9) vTaskDelay(pdMS_TO_TICKS(1));
            }
            ...
            for (int strip = 0; strip < strip_num; strip++){
                ...
                for (int led = 0; led < led_num; led++){
                    ...
                    if (led % 5 == 4) vTaskDelay(pdMS_TO_TICKS(1));
                }
...
void app_main(void){
    ...
    //配置 SD卡插槽 跟 FAT檔案系統掛載
    sdmmc_slot_config_t slot_config = SDMMC_SLOT_CONFIG_DEFAULT();
    slot_config.width = 4;  //4-line
    ...
    if (ret != ESP_OK){
        ...
        else ESP_LOGE(TAG, "Failed to initialize SD card (%s)", esp_err_to_name(ret));
        return;
    }
    ...
    if (f == NULL) ESP_LOGE(TAG, "/sd/frame.dat not found!");
    else{
